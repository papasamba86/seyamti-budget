...
import { ressourceSchema } from '@/lib/validations';
...
export async function PUT(req: Request) {
  ...
  const rows = await sql`
    ...
    RETURNING *
  `;
  if (rows.length === 0) return apiError('Ressource introuvable', 404);
  return apiOk(rows[0]);
}
...
export async function DELETE(req: Request) {
  ...
  const sql = getDb();
  await sql`DELETE FROM ressources WHERE id = ${parseInt(id)}`;
